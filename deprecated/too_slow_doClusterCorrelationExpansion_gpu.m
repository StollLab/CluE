...
for iCluster=1:numberClusters(cluster_order)
  ...
  for iorder=cluster_order:maxSize
    ...
    Signals(iorder,:) =  Signals(iorder,:) ...
      .*(1 + isotopeProbability*(AuxiliarySignal_1(iCluster,:) -1) );
...
for iCluster=1:numberClusters(cluster_order)
  ...
  for subClusterSize= 1:cluster_order-1
    ...
    for subcluster_index = SubclusterIndices(:,subClusterSize,iCluster)'
      ...
      AuxSig_(iCluster,:) = AuxSig_(iCluster,:)./aux_(subcluster_index,:);
      
    end
    
    switch cluster_order
      case 2
        AuxiliarySignal_2 = AuxSig_;
      case 3
        AuxiliarySignal_3 = AuxSig_;
      case 4
        AuxiliarySignal_4 = AuxSig_;
      case 5
        AuxiliarySignal_5 = AuxSig_;
      case 6
        AuxiliarySignal_6 = AuxSig_;
    end
  end
  ...
  for iorder=cluster_order:maxSize
    ...
    switch cluster_order
      case 2
        Signals(iorder,:) =  Signals(iorder,:) ...
          .*(1 + isotopeProbability*(AuxiliarySignal_2(iCluster,:) -1) );
      case 3
        Signals(iorder,:) =  Signals(iorder,:) ...
          .*(1 + isotopeProbability*(AuxiliarySignal_3(iCluster,:) -1) );
      case 4
        Signals(iorder,:) =  Signals(iorder,:) ...
          .*(1 + isotopeProbability*(AuxiliarySignal_4(iCluster,:) -1) );
      case 5
        Signals(iorder,:) =  Signals(iorder,:) ...
          .*(1 + isotopeProbability*(AuxiliarySignal_5(iCluster,:) -1) );
      case 6
        Signals(iorder,:) =  Signals(iorder,:) ...
          .*(1 + isotopeProbability*(AuxiliarySignal_6(iCluster,:) -1) );
      end
  end
end
