...
%     skipCluster = (~all(state_multiplicity(thisCluster)==state_multiplicity(thisCluster(1)))) ...
    ...
    if useInterlacedClusters(Method_order,clusterSize)
      ...
      [tensors,zeroIndex] = pairwisetensors(Nuclei_g, Nuclei_Coordinates,thisCluster,Atensors,magneticField, ge,geff, muB, muN, mu0, hbar,theory);
    ...
    % Loop over cyclic permutations.
      for iPerm = 1:nPerm
        ...
        % Build Hamiltonians.
        [H_alpha,H_beta] = ...
          assembleHamiltonian(state_multiplicity(thisCluster),tensors,SpinOp,qtensors,SpinXiXjOp,...
...
%             [H_alphaMF,H_betaMF] = assembleMeanFieldHamiltonian(state_multiplicity(thisCluster),tensors,SpinOp,qtensors,SpinXiXjOp,...
